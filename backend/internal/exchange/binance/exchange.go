...
// Exchange implements exchange interfaces for Binance
...
// New creates a new Binance exchange client
func New(apiKey, secretKey string, useTestnet bool) (*Exchange, error) {
...
func (be *Exchange) SetClient(client *binance.Client) {
	be.client = client
	if be.prices == nil {
...
// SetCollector sets the collector for historical data collection
func (be *Exchange) SetCollector(candleRepo CandleRepository) {
	be.collector = NewCollector(be.client, candleRepo)
}

// CollectHistorical collects historical candle data for a symbol
func (be *Exchange) CollectHistorical(ctx context.Context, symbol, interval string, startTime, endTime time.Time) error {
	if be.collector == nil {
		return fmt.Errorf("collector not initialized, call SetCollector first")
	}
	return be.collector.CollectHistorical(ctx, symbol, interval, startTime, endTime)
}

// GetCurrentPrice returns the current market price (always fetches fresh data from API)
func (be *Exchange) GetCurrentPrice(ctx context.Context, symbol string) (float64, error) {
...
func (be *Exchange) GetCandles(ctx context.Context, symbol, interval string, limit int) ([]*domain.Candle, error) {
...
func (be *Exchange) PlaceOrder(ctx context.Context, order *domain.Order) (*domain.Order, error) {
	be.mu.RLock()
	if be.closed {
		be.mu.RUnlock()
...
func (be *Exchange) GetOrder(ctx context.Context, orderID string) (*domain.Order, error) {
...
func (be *Exchange) CancelOrder(ctx context.Context, orderID string) error {
	_, err := strconv.ParseInt(orderID, 10, 64)
	if err != nil {
...
func (be *Exchange) StreamKlines(ctx context.Context, symbol, interval string, callback func(*domain.Candle)) error {
...
func (be *Exchange) SubscribePrice(ctx context.Context, symbol string) (<-chan float64, error) {
	be.mu.Lock()
	defer be.mu.Unlock()
...
func (be *Exchange) startPriceWebSocket(symbol string) {
