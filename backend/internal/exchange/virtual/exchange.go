...
// Exchange implements a virtual exchange for testing and simulation
...
// New creates a new virtual exchange
func New(initialPrices map[string]float64) *Exchange {
...
func (ve *Exchange) simulatePriceUpdates() {
...
func (ve *Exchange) checkLimitOrders() {
	ve.mu.Lock()
	defer ve.mu.Unlock()
...
func (ve *Exchange) fillOrder(order *domain.Order, price float64) {
	now := time.Now()
...
func (ve *Exchange) GetCurrentPrice(ctx context.Context, symbol string) (float64, error) {
	ve.mu.RLock()
	defer ve.mu.RUnlock()
...
func (ve *Exchange) GetCandles(ctx context.Context, symbol, interval string, limit int) ([]*domain.Candle, error) {
...
func (ve *Exchange) PlaceOrder(ctx context.Context, order *domain.Order) (*domain.Order, error) {
	ve.mu.Lock()
	defer ve.mu.Unlock()
...
func (ve *Exchange) GetOrder(ctx context.Context, orderID string) (*domain.Order, error) {
	ve.mu.RLock()
	defer ve.mu.RUnlock()
...
func (ve *Exchange) CancelOrder(ctx context.Context, orderID string) error {
	ve.mu.Lock()
	defer ve.mu.Unlock()
...
// CollectHistorical is a no-op for virtual exchange
func (ve *Exchange) CollectHistorical(ctx context.Context, symbol, interval string, startTime, endTime time.Time) error {
	// Virtual exchange doesn't need to collect historical data
	return nil
}

// StreamKlines streams real-time kline/candle data (stub for virtual exchange)
func (ve *Exchange) StreamKlines(ctx context.Context, symbol, interval string, callback func(*domain.Candle)) error {
	// For virtual exchange, we can simulate kline updates
	ticker := time.NewTicker(1 * time.Second)
	defer ticker.Stop()
	...
	for {
		...
		case <-ticker.C:
			...
			candle := &domain.Candle{
				Symbol:    symbol,
				OpenTime:  now,
				CloseTime: now.Add(1 * time.Second),
				Open:      price,
				High:      price * 1.001,
				Low:       price * 0.999,
				Close:     price,
...
func (ve *Exchange) SubscribePrice(ctx context.Context, symbol string) (<-chan float64, error) {
	ve.mu.Lock()
	defer ve.mu.Unlock()
...
func (ve *Exchange) SetPrice(symbol string, price float64) {
	ve.mu.Lock()
	defer ve.mu.Unlock()
